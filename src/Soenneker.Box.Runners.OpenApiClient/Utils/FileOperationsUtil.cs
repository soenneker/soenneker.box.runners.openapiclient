...
using Soenneker.Box.Runners.OpenApiClient.Utils.Abstract;
...
namespace Soenneker.Box.Runners.OpenApiClient.Utils;
...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string openApiDocumentUrl = _configuration["Box:ClientGenerationUrl"] ?? "https://raw.githubusercontent.com/box/box-openapi/refs/heads/main/openapi.json";
        ...
        if (filePath is null)
            throw new InvalidOperationException("The Box OpenAPI document could not be downloaded.");
        ...
        await _kiotaUtil.Generate(filePath, "BoxOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();
    ...
    private async ValueTask BuildAndPush(string gitDirectory, CancellationToken cancellationToken)
    {
        ...
        if (!successful)
        {
            throw new InvalidOperationException("The generated Box client failed its Release build.");
        }
